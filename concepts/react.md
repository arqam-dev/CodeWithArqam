...
--> General Notes:
    - React is a front-end library developed by Facebook (now Meta)
...
→ Diffing Algorithm (run on change or prop or state):
...
→ Reconciliation:
    - Overall process of updating the DOM when component state or props change
    - Uses Diffing Algorithm to decide what and how to update efficiently
    -> Detailed Steps how it updates Virtual and Real DOM:
        ...
        -> Step 2: When does React update the Real DOM:
            ...
            -> Sumary:
                - Legacy React: 
                  - Combined updates only inside event handlers.
                  - Painting the whole wall in one go → user can’t do anything until painting finishes. although can batch events inside an event handler to acheive a little efficiency.
                - React 18+: 
                  - React batches all updates that happen close together — within the same logical block async function, or event loop tick.
                    - Painting one section at a time → user can still interact with the app while painting continues.
...
→ React Fiber:
    ...
    - Can pause, resume, or reuse work → better performance in complex apps:
        - like If the browser is busy (user typing, scrolling), React can pause rendering, let the browser handle it, then resume later.
...
→ Hooks:
    ...
    - Core Hooks:
        ...
        - useEffect():
            ...
            - Syntax: Syntax: useEffect(callback, dependencies)
            - Example:
                useEffect(() => {
            ...
            - Example:
                ...
                const timer = setInterval(() => console.log("Running..."), 1000); // any interval we set for our usecase
...
→ Prop VS Key Prop VS Render Prop:
    - Prop:
...
--> Props VS State (Detailed):
    ...
    - Conditions:
        - Initial value from parent: Props Yes, State Yes
        - Changed by parent: Props Yes, State No
        - Default values inside component: Props Yes, State Yes
        - Change inside component: Props No, State Yes
        - Set initial value for child: Props Yes, State Yes
        - Change in child: Props Yes, State No
    - Similarities:
...
→ Higher-Order Components (HOCs):
    ...
    - It wraps it and adds extra behavior without updating the amin component (like a Decorator).
    - Example scenarios:
